...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        function init_sub_table(){
            $('body').on('click','#exampleTable tbody td img', function(){
                ...
                if (oTable.fnIsOpen(nTr)) {
                    ...
                    oInnerTable = $("#exampleTable_" + iTableCounter).dataTable({
                        ...
                        "aoColumns": [
                            ...
                            { "mDataProp": "notes" },
                            { "mDataProp": "source" },
                            { "sDefaultContent": "<a data-toggle='modal' href='#edit-indicator-modal' class='edit-modal-indicator' id=''><i class='fa fa-pencil-square-o fa-lg edit-indicator' aria-hidden='true'></i></a>" + "<a href='#'><i class='fa fa-trash-o fa-lg remove-indicator' aria-hidden='true'></i></a>" },
                        ],
                        "bPaginate": false,
                        "oLanguage": {
                            "sInfo": "_TOTAL_ entries"
        ...
        $('body').on('click','.show-sub-table', function(){
            var indicator_id = $(this).attr('id');
            var sdg_id = $(this).attr('data-sdg');
        ...
        $('#add-measurement-modal').on('submit',function(e){
            ...
            $.ajax({
                ...
                type: 'POST', //or POST
                dataType: 'json',
                data: {'m-sdg':$('#measurement_sdg').val(),'indicator_id':$('#measurement_indicator_id').val(),'date-m':$('#date-measurement').val(),'value-m':$('#value-measurement').val(),'target-value-measurement':$('#target-value-measurement').val(),'notes':$("#notes-measurement").val(),'source-m':'#source-measurement','action-measurement':'add-measurement'},
    ...
    $('#edit-indicator-form').on('submit',function(e){
        ...
        $.ajax({
            ...
            type: 'POST', //or POST
            dataType: 'json',
            data: {'indicator_id':$('#edit_indicator_id').val(),'description':$('#edit-sdg-description').val(),'indicator':$('#edit_indicator').val(),'sdg':$("#edit-sdg-type").children(":selected").attr("id"),'edit_action_indicator':'edit_indicator_form'},
            success: function(data){
    ...
    $('#add-indicator-form').on('submit', function(e){
        ...
        $.ajax({
            ...
            type: 'POST', //or POST
            dataType: 'json',
            data: {'description':$('#sdg-description').val(),'indicator':$('#indicator').val(),'sdg':$("#sdg-type").children(":selected").attr("id"),'action_indicator':'add_indicator'},
            success: function(data){
                $('#exampleTable').dataTable().fnDestroy();
                init_table(data);
